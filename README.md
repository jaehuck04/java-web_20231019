...
12주차 마무리 과제: 추가 구현  
(1)로그인 에러 처리 - 로그인 실패 시 오류 메시지 출력  
(2)업로드 에러 처리 - 사진 업로드에 대한 오류 메시지 없음  
...
sessionStorage.setItem, sessionStorage.getItem을 활용하여   
메인페이지에서 다운로드 페이지, 로그인 페이지 등 페이지 이동 시  
다크/라이트 모드 유지 구현을 완료함  
